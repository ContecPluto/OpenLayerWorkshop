...
function ol_coordinate_dist2d(p1, p2) {
  const dx = p1[0] - p2[0];
  const dy = p1[1] - p2[1];
 ...
 */
const ol_graph_Dijskra = class olgraphDijskra extends ol_Object {
  constructor(options) {
    options = options || {};
    super();
   ...
   */
  getEdges(coord) {
    const extent = ol_extent_buffer(
      ol_extent_boundingExtent([coord]),
      this.get('epsilon')
    );
    const result = [];
  ...
  addNode(p, wdist, dist, from, prev) {
    ...
    const dtotal =
      wdist +
      this.getLength(new ol_geom_LineString([this.end, p])) * this.weight();
   ...
   */
  closestCoordinate(p) {
    const e = this.source.getClosestFeatureToCoordinate(p);
    const p0 = e.getGeometry().getFirstCoordinate();
    const p1 = e.getGeometry().getLastCoordinate();
  ...
  _resume() {
    ...
    while (this.candidat.length) {
      ...
      this.candidat.sort(function (a, b) {
        return a.get('dtotal') < b.get('dtotal')
          ? 1
          : a.get('dtotal') === b.get('dtotal')
          ? 0
          : -1;
      });

      // First candidate
      const node = this.candidat.pop();
      const p = node.getGeometry().getCoordinates();
      // Find connected edges
      const edges = this.getEdges(p);

      for (var i = 0, e; (e = edges[i]); i++) {
        if (node.get('from') !== e) {
          let dist = this.getLength(e);
          if (dist < 0) {
            console.log('distance < 0!');
            // continue;
          }
          const wdist = node.get('wdist') + dist * this.weight(e);
          dist = node.get('dist') + dist;
          const pt1 = e.getGeometry().getFirstCoordinate();
          const pt2 = e.getGeometry().getLastCoordinate();
          const sens = this.direction(e);
          if (sens !== 0) {
