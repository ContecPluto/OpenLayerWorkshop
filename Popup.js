...
import ol_ext_element from './utill_ele.js';
 ...
 * @api stable
 */
const ol_Overlay_Popup = class olOverlayPopup extends ol_Overlay {
  constructor(options) {
    options = options || {};
    // Popup div
   ...
   */
  setPopupClass(c) {
    const classes = ['ol-popup'];
    if (this.getVisible()) {
      classes.push('visible');
    }
    this.element.className = '';
    const classesPositioning = this.getClassPositioning()
      .split(' ')
  ...
  setPositioning_(pos) {
    if (this.element) {
      ...
      const classes = this.getClassPositioning()
        .split(' ')
        .filter(function (className) {
          return className.length > 0;
        });
  ...
  show(coordinate, html) {
    ...
    if (html && html !== this.prevHTML) {
      ...
      Array.prototype.slice
        .call(this.content.querySelectorAll('img'))
    ...
    if (coordinate) {
      ...
      if (this.autoPositioning) {
        const p = map.getPixelFromCoordinate(coordinate);
        const s = map.getSize();
        const pos = [];
        ...
        if (this.offsetBox) {
          this.setOffset([
            this.offsetBox[pos[1] == 'left' ? 2 : 0],
            this.offsetBox[pos[0] == 'top' ? 3 : 1],
          ]);
        }
      } else {
        if (this.offsetBox) {
