...
import ol_ext_input_Radio from './input_rad.js';
import ol_ext_input_Switch from './input_swi.js';
...
ol_ext_element.createSwitch = function (options) {
  ...
  const opt = Object.assign({input: input}, options || {});
...
ol_ext_element.getFixedOffset = function (elt) {
  ...
  var getOffset = function (parent) {
    ...
    if (
      ol_ext_element.getStyle(parent, 'position') === 'absolute' &&
 ...
 * @return {Object} an object with a refresh function
 */
ol_ext_element.scrollDiv = function (elt, options) {
  ...
  const onmove =
    typeof options.onmove === 'function' ? options.onmove : function () {};
  ...
  var onPointerMove = function (e) {
    if (pos !== false) {
      const delta = (isbar ? -1 / scale : 1) * (pos - e[page]);
  ...
  if (options.vertical && options.minibar) {
    ...
    // or wait when ready
      ...
      elt.addEventListener('pointermove', init);
    }
    // Update on scroll
